...
class TestMainDatabase(MainDBTemplate):
    ...
    def test_add_get_update_and_delete_note(self):
        ...
        self.assertFalse(MainDB.delete_user_note(42, 1), msg="user don't exist")
        ...
        self.assertFalse(MainDB.delete_user_note(uid, note_num), msg="note don't exist")
        ...
        self.assertEqual(note_q1.text, note_text)

        new_note_text = 'world'
        self.assertTrue(MainDB.update_user_note(uid, note_num, new_note_text))
        note_q2 = MainDB.get_user_note(uid, note_num)
        self.assertEqual(note_q2.text, new_note_text)

        self.assertTrue(MainDB.delete_user_note(uid, note_num))
    ...
    def test_get_users_and_notes_lists(self):
        ...
        self.assertEqual(users[0].tg_id, uid1)
...
class TestMainDatabaseDeeper(MainDBTemplate):
    ...
    def test_db_add_and_get_notes(self):
        ...
        self.assertEqual(u2_notes[0].text, 'strongest')
        self.assertEqual(u2_notes[1].text, 'genius')
    ...
    def test_db_delete_note(self):
        self.assertFalse(MainDB.delete_user_note(uid=0, note_num=0))
        uid = 20
        ...
        self.assertEqual(notes_q1[0].text, 'hello')
        self.assertEqual(notes_q1[1].text, 'world')
        self.assertEqual(notes_q1[0].number, 1)
        self.assertEqual(notes_q1[1].number, 2)

        # delete 1st entry
        MainDB.delete_user_note(uid, 1)
        ...
        self.assertEqual(notes_q2[0].text, 'world')
        ...
        self.assertEqual(notes_q3[0].text, 'world')
        self.assertEqual(notes_q3[1].text, 'foobar')
        self.assertEqual(notes_q3[0].number, 2)
        self.assertEqual(notes_q3[1].number, 3)

        # delete the 3rd entry and add a 4th
        MainDB.delete_user_note(uid, 3)
        ...
        self.assertEqual(notes_q4[0].text, 'world')
        self.assertEqual(notes_q4[1].text, 'bazqux')
    ...
    def test_db_update_note(self):
        ...
        self.assertEqual(len(notes_q1), 2)
        self.assertEqual(notes_q1[0].text, 'foo')
        self.assertEqual(notes_q1[1].text, 'bar')
        self.assertEqual(notes_q1[0].number, 1)
        self.assertEqual(notes_q1[1].number, 2)
        ...
        self.assertEqual(notes_q2[0].text, 'baz')
        self.assertEqual(notes_q2[1].text, 'bar')
...
class TestMainDatabaseUpdaterLogic(MainDBTemplate):
    ...
    def test_updater_methods_first(self):
        ...
        self.assertEqual(MainDB.get_notifications_for_the_updater(42), [])
        self.assertFalse(MainDB.update_user_last_notes_list(42, 1))
        ...
        notes_q1 = MainDB.get_notifications_for_the_updater(uid)
        self.assertIsNotNone(notes_q1)
        self.assertEqual(len(notes_q1), 3, msg='notes == user.notes')

        last_note = notes_q1[0].number
        self.assertTrue(MainDB.update_user_last_notes_list(uid, last_note), msg='user.notes_total <= LAST_NOTES_LIST')

        notes_q2 = MainDB.get_notifications_for_the_updater(uid)
    ...
    def test_updater_methods_second(self):
        ...
        notes_q1 = MainDB.get_notifications_for_the_updater(uid)
        self.assertIsNotNone(notes_q1)
        self.assertEqual(len(notes_q1), 4, msg='user.notes_total > LAST_NOTES_LIST and NOT user.last_notes')

        last_note = notes_q1[0].number
        self.assertTrue(MainDB.update_user_last_notes_list(uid, last_note), msg='user.last_notes is None')
    ...
    def test_updater_methods_third(self):
        ...
        self.assertEqual(notes_q1[0].text, 'four')
        self.assertEqual(notes_q1[0].number, 4)

        MainDB.update_user_last_notes_list(uid, notes_q1[0].number)
        notes_q2 = MainDB.get_notifications_for_the_updater(uid)
        self.assertEqual(len(notes_q2), 1)
        self.assertEqual(notes_q2[0].text, 'one')
        self.assertEqual(notes_q2[0].number, 1)

        MainDB.update_user_last_notes_list(uid, notes_q2[0].number)
        notes_q3 = MainDB.get_notifications_for_the_updater(uid)
        self.assertEqual(len(notes_q3), 1)
        self.assertEqual(notes_q3[0].text, 'two')
        self.assertEqual(notes_q3[0].number, 2)

        MainDB.update_user_last_notes_list(uid, notes_q3[0].number)
        notes_q4 = MainDB.get_notifications_for_the_updater(uid)
        self.assertEqual(len(notes_q4), 1)
        self.assertEqual(notes_q4[0].text, 'three')
        self.assertEqual(notes_q4[0].number, 3)

        MainDB.update_user_last_notes_list(uid, notes_q4[0].number)
        notes_q5 = MainDB.get_notifications_for_the_updater(uid)
        self.assertEqual(len(notes_q5), 1)
        self.assertEqual(notes_q5[0].text, 'four')
        self.assertEqual(notes_q5[0].number, 4)

        self.assertTrue(MainDB.update_user_last_notes_list(uid, notes_q5[0].number))
